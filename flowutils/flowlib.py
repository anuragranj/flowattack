...
def interp_gt_flow(mat, mask=None):
    # import pdb; pdb.set_trace()
    if mask is not None:
        invalid_idx = (mask == 0)
        mat[invalid_idx, 0] = np.nan
        mat[invalid_idx, 1] = np.nan
    ...
    x1 = xx[~mat.mask[:,:,0]]
    y1 = yy[~mat.mask[:,:,1]]
    newarr = mat[~mat.mask].reshape(-1,2)

    GD1 = interpolate.griddata((x1, y1), newarr, (xx, yy), method='linear', fill_value=1e8)
    return GD1 
